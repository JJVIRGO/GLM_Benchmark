...
"""
Fine-tune Nucleotide Transformer (InstaDeepAI/nucleotide-transformer-v2-500m-multi-species)
with optional DNase-seq signal fusion for cross-cell-type TF binding prediction.

NT uses overlapping 6-mers with stride 1; offset mapping is built manually so
DNase signals can be aligned to the correct base positions.
"""
...
class ModelArguments:
    model_name:      str  = field(default="NT")
    model_path:      str  = field(default="InstaDeepAI/nucleotide-transformer-v2-500m-multi-species")
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    run_name:                    str  = field(default="exp3_nt_run")
    optim:                       str  = field(default="adamw_torch")
    model_max_length:            int  = field(default=171)
    ...
    output_dir:                  str  = field(default="./outputs/exp3_nt")
...
class CrossCellNTDataset(Dataset):
    ...
    def _process_sequence(self, dna_seq: str):
        ...
        for tok in tokens:
            if tok in special:
                ...
                pos += 1  # NT uses overlapping k-mers with stride 1
...
class CrossCellNTClassifier(nn.Module):
    def __init__(self, model_name: str, model_path: str, hidden_dim: int, freeze_backbone: bool = True):
        super().__init__()
...
# ---------------------------------------------------------------------------
# Metrics  (identical across all finetune scripts)
# ---------------------------------------------------------------------------
...
def train():
    ...
    print(f"Loading tokenizer: {model_args.model_path}")
    tokenizer = AutoTokenizer.from_pretrained(
        model_args.model_path, trust_remote_code=True,
        model_max_length=training_args.model_max_length,
    )

    train_ds = CrossCellNTDataset(data_args.train_data_path, tokenizer, "train")
    val_ds   = CrossCellNTDataset(data_args.val_data_path,   tokenizer, "val")
    test_ds  = CrossCellNTDataset(data_args.test_data_path,  tokenizer, "test")

    model = CrossCellNTClassifier(
    ...
    trainer = transformers.Trainer(
        ...
        data_collator=DataCollatorForNT(tokenizer),
    )
